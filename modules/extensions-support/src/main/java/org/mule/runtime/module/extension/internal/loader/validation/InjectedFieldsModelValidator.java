        ...
        new ExtensionWalker() {
                  ...
                  if (!objectType.getAnnotation(InfrastructureTypeAnnotation.class).isPresent()) {
                    try {
                      ...
                      if (validatedTypes.add(type)) {
                        validateType(model, type, DefaultEncoding.class, org.mule.sdk.api.annotation.param.DefaultEncoding.class);
                      }
                    } catch (Exception e) {
          ...
          private void validateFields(NamedObject model, Optional<Class> implementingType,
            ...
            implementingType.ifPresent(type -> {
              validateType(model, type, annotationClasses);
            });
          }

          private void validateType(NamedObject model, Class<?> type, Class<? extends Annotation>... annotationClasses) {
